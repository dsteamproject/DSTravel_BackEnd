...
public interface TDRepository extends JpaRepository<TD, Integer> {
        ...
        // 가격별 전체 숙소 조회
        @Query(value = "SELECT * FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND TDTYPE=32", nativeQuery = true)
        public List<TD> SelectHOTEL(@Param("firstprice") int firstprice, @Param("endprice") int endprice,
                        Pageable pageable);

        @Query(value = "SELECT COUNT(*) FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND TDTYPE=32", nativeQuery = true)
        public int COUNTSelectHOTEL(@Param("firstprice") int firstprice, @Param("endprice") int endprice);

        // 가격별, 지역별 숙소 조회
        @Query(value = "SELECT * FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND CITY=:city AND TDTYPE=32", nativeQuery = true)
        ...
        @Query(value = "SELECT COUNT(*) FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND CITY=:city AND TDTYPE=32", nativeQuery = true)
        public int COUNTSelectHOTELBYCITY(@Param("firstprice") int firstprice, @Param("endprice") int endprice,
                        @Param("city") Integer city);

        // 가격별, 등급별 숙소 조회
        @Query(value = "SELECT * FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND RANK=:rank AND TDTYPE=32", nativeQuery = true)
        ...
        @Query(value = "SELECT COUNT(*) FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND RANK=:rank AND TDTYPE=32", nativeQuery = true)
        public int COUNTSelectHOTELBYRANK(@Param("firstprice") int firstprice, @Param("endprice") int endprice,
                        @Param("rank") Integer rank);

        // 지역별, 가격별, 등급별 숙소 조회
        @Query(value = "SELECT * FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND CITY=:city AND RANK=:rank AND TDTYPE=32", nativeQuery = true)
        ...
        @Query(value = "SELECT COUNT(*) FROM TD WHERE PRICE>=:firstprice AND PRICE<=:endprice AND CITY=:city AND RANK=:rank AND TDTYPE=32", nativeQuery = true)
        ...
        // ----------------selectAll--------------------
        // 전체 여행지,숙소,검색 조회
        @Query(value = "SELECT * FROM TD WHERE Title LIKE '%' || :title || '%' AND TDTYPE=:contentTypeId AND CITY=:areaCode AND STATE=1", nativeQuery = true)
        ...
        // 전체 여행지,숙소,검색 조회 수
        @Query(value = "SELECT COUNT(*) FROM TD WHERE Title LIKE '%' || :title || '%' AND TDTYPE=:contentTypeId AND CITY=:areaCode AND STATE=1", nativeQuery = true)
        public int CountSelectTD(@Param("title") String title, @Param("areaCode") String areaCode,
                        @Param("contentTypeId") String contentTypeId);

        // 전체 여행지,숙소,검색 조회 + 임시여행지
        @Query(value = "SELECT * FROM TD WHERE Title LIKE '%' || :title || '%' AND TDTYPE=:contentTypeId AND CITY=:areaCode AND STATE=1 OR STATE=0 AND USERS=:id", nativeQuery = true)
        ...
        @Query(value = "SELECT COUNT(*) FROM TD WHERE Title LIKE '%' || :title || '%' AND TDTYPE=:contentTypeId AND CITY=:areaCode AND STATE=1 OR STATE=0 AND USERS=:id", nativeQuery = true)
        ...
        // ---------------------좋아요한 여행지, 숙소, 음식점--------------------------
        @Query(value = "SELECT * FROM TD WHERE NO=:td AND TDTYPE=:Type AND STATE=1", nativeQuery = true)
        public TD selectGoodType(@Param("td") Integer td, @Param("Type") TDType Type);

        // ---------------------내가 요청한 지도(여행지)-------------------------------
        @Query(value = "SELECT * FROM TD WHERE USERS=:id", nativeQuery = true)
        public List<TD> selectMyTDtem(@Param("id") String id, Pageable pageable);

        @Query(value = "SELECT COUNT(*) FROM TD WHERE USERS=:id", nativeQuery = true)
        ...
        // ---------------------여행지 임시저장 ADMIN List----------------------------
        @Query(value = "SELECT * FROM TD WHERE USERS IS NOT NULL AND USERS !='ADMIN' AND STATE=:state", nativeQuery = true)
        public List<TD> selectAdminTDtem(@Param("state") int state);

        // -----------------------WORLD CUP 조회 ----------------------
        @Query(value = "SELECT * FROM TD WHERE CITY=:city AND TDTYPE=:type AND STATE=1 ", nativeQuery = true)
        public List<TD> selectTDWorldCup(@Param("city") City city, @Param("type") TDType tpye);

        @Query(value = "SELECT COUNT(*) FROM TD WHERE CITY=:city AND TDTYPE=:type AND STATE=1 ", nativeQuery = true)
        public int COUNTTDWorldCup(@Param("city") City city, @Param("type") TDType type);
